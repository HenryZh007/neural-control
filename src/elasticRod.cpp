...
void elasticRod::setDOFBoundaryCondition(double desiredValue, int k)
{
	// k is the index in the full dof vector
	isConstrained[k] = 1;
	x(k) = desiredValue;
}
...
MatrixXd elasticRod::computeCurvature(const MatrixXd & vertices)
{
	int num_v = vertices.rows();
	MatrixXd curvatures(num_v, 2);
	curvatures = MatrixXd::Zero(num_v, 2);
	...
	{
		Vector3d v0 = vertices.row(i-1);
		Vector3d v1 = vertices.row(i);
		Vector3d v2 = vertices.row(i+1);
		...
		Vector3d kb = 2.0 * t0.cross(t1) / (1.0+t0.dot(t1));

		m1e=m1.row(i-1);
        m2e=m2.row(i-1);
        m1f=m1.row(i);
        m2f=m2.row(i);
		curvatures(i, 0)= 0.5 * kb.dot(m2e + m2f);
		curvatures(i, 1)=-0.5 * kb.dot(m1e + m1f);
...
void elasticRod::updateTimeStep()
{
	...
	// u = VectorXd::Zero(ndof); // reset velocity to zero
...
VectorXd elasticRod::getFreeDOF()
{
	VectorXd xUncons(uncons);
	for (int c=0; c < uncons; c++)
	{
		xUncons[c] = x[unconstrainedMap[c]];
	}

	return xUncons;
}

void elasticRod::setFreeDOF(const VectorXd &x_free)
{
	for (int c=0; c < uncons; c++)
	{
		x[unconstrainedMap[c]] = x_free[c];
	}
}