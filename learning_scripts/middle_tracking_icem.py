...
CONFIG = {
    ...
    "iteration_number": 500,          # ICEM iterations (same as epoch count in noMPC)
    "loss_threshold": 1e-7,
    ...
    "noise_beta": 1.0,              # Temporal correlation of ICEM samples
...
def init_position_history_file(filepath: str, case_idx: int, traj_type: str, target_index: int):
    with open(filepath, "w") as f:
        f.write("# Per-step middle point position history for middle_tracking_icem\n")
        f.write(f"# Case {case_idx}: Trajectory type = {traj_type}\n")
        f.write(f"# Target node index: {target_index}\n")
        f.write("# Format: Epoch, TimeStep, X, Y\n")


def append_epoch_positions(filepath: str, epoch_idx: int, epoch_positions):
...
# =============================================================================
# ICEM optimizer for trajectory tracking
# =============================================================================
def icem_optimize_trajectory(
    sim_manager,
    T: int,
    target: np.ndarray,       # (T, 2) - target trajectory
    target_index: int,
    ...
    position_output_file: str | None = None,
):
    """
    ICEM over policy parameters for trajectory tracking using colored-noise
    sampling and elite reuse.
    ...
    for it in range(icem_iters):
        ...
        if best_positions is not None:
            ...
            if position_output_file is not None:
                append_epoch_positions(position_output_file, it, best_positions)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print(f"Testing {len(trajectory_types)} trajectory types with ICEM")
    print(f"{'='*60}\n")
    ...
    for traj_idx, trajectory_type in enumerate(trajectory_types):
        ...
        print(f"  Number of ICEM iterations: {iteration_number}")
        print(f"  Population size: {popsize}")
        print(f"  Elite fraction: {elite_frac}")
        print(f"{'='*60}\n")

        pos_file = os.path.join(script_dir, f"middle_tracking_icem_case{traj_idx}_{trajectory_type}_positions.txt")
        init_position_history_file(pos_file, traj_idx, trajectory_type, target_index)

        # Run ICEM optimization
        total_start_time = time.perf_counter()

        best_u, best_loss, mu, std, hist, position_history = icem_optimize_trajectory(
        ...
        print(f"\n{'='*60}")
        print(f"[{trajectory_type}] ICEM optimization completed!")
    ...
    output_file = os.path.join(script_dir, "middle_tracking_icem.txt")

    # Save summary to txt file
    with open(output_file, "w") as f:
        f.write("="*60 + "\n")
        f.write("Summary Table (ICEM):\n")
        f.write("="*60 + "\n")
        f.write(f"ICEM population size: {popsize}\n")
        f.write(f"ICEM elite fraction: {elite_frac}\n")
        f.write("-"*65 + "\n")
